...
if (process.env.NODE_ENV !== 'production') {
  host = 'https://nft-linter.theqrl.org';
}
...
function formatMessages(messages) {
  const errors = messages.map((message) => {
    return `L${message.line}:${message.column} ${message.message.slice(0, -1)}`;
  });
...
app.get('/', (req, res) => {
  res.send(
    ...
    console.log(data);\
        let response = await fetch("${host}/lint", { \
                method: "POST", \
                headers: { \
...
// POST must have valid json in body
app.post('/lint-stats', async (req, res) => {
  const json = req.body;
  let isValid = false;
  isValid = await lintOnly(json);
  res.send(isValid);
});
